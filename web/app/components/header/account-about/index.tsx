...
import type { ChainAINexusVersionResponse } from '@/models/common'
import { IS_CE_EDITION } from '@/config'
import LogoSite from '@/app/components/base/logo/logo-site'

type IAccountSettingProps = {
  chainainexusVersionInfo: ChainAINexusVersionResponse
  onCancel: () => void
}
...
export default function AccountAbout({
  chainainexusVersionInfo,
  onCancel,
}: IAccountSettingProps) {
  const { t } = useTranslation()
  const isLatest = chainainexusVersionInfo.current_version === chainainexusVersionInfo.latest_version
    ...
    >
      <div className='relative pt-4'>
        ...
        <div>
          ...
          <div className='mb-3 text-center text-xs font-normal text-gray-500'>Version {chainainexusVersionInfo?.current_version}</div>
          <div className='mb-4 text-center text-xs font-normal text-gray-700'>
            <div>© {dayjs().year()} ChainAINexus, Inc., Contributors.</div>
        ...
        <div className='flex justify-between items-center'>
          <div className='text-xs font-medium text-gray-800'>
            {
              isLatest
                ? t('common.about.latestAvailable', { version: chainainexusVersionInfo.latest_version })
                : t('common.about.nowAvailable', { version: chainainexusVersionInfo.latest_version })
            }
          </div>
          <div className='flex items-center'>
            <Link
              className={classNames(buttonClassName, 'mr-2')}
              href={'https://github.com/chainainexus/can20/releases'}
            ...
            </Link>
            {
              !isLatest && !IS_CE_EDITION && (
                <Link
                  ...
                  href={chainainexusVersionInfo.release_notes}
                  target='_blank' rel='noopener noreferrer'
                >
