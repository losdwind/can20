...
def send_invite_member_mail_task(language: str, to: str, token: str, inviter_name: str, workspace_name: str):
    ...
    try:
        url = f"{can20_config.CONSOLE_WEB_URL}/activate?token={token}"
        if language == "zh-Hans":
            ...
            mail.send(to=to, subject="立即加入 CAN20 工作空间", html=html_content)
        else:
            html_content = render_template(
            ...
            mail.send(to=to, subject="Join CAN20 Workspace Now", html=html_content)
