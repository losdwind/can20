...
class AliyunOssStorage(BaseStorage):
    ...
    def __init__(self):
        super().__init__()
        self.bucket_name = can20_config.ALIYUN_OSS_BUCKET_NAME
        self.folder = can20_config.ALIYUN_OSS_PATH
        oss_auth_method = aliyun_s3.Auth
        region = None
        if can20_config.ALIYUN_OSS_AUTH_VERSION == "v4":
            oss_auth_method = aliyun_s3.AuthV4
            region = can20_config.ALIYUN_OSS_REGION
        oss_auth = oss_auth_method(can20_config.ALIYUN_OSS_ACCESS_KEY, can20_config.ALIYUN_OSS_SECRET_KEY)
        self.client = aliyun_s3.Bucket(
            oss_auth,
            can20_config.ALIYUN_OSS_ENDPOINT,
            self.bucket_name,
            connect_timeout=30,
            region=region,
        )
