...
class HostingConfiguration:
    ...
    def init_azure_openai() -> HostingProvider:
        ...
        if can20_config.HOSTED_AZURE_OPENAI_ENABLED:
            credentials = {
                "openai_api_key": can20_config.HOSTED_AZURE_OPENAI_API_KEY,
                "openai_api_base": can20_config.HOSTED_AZURE_OPENAI_API_BASE,
                "base_model_name": "gpt-35-turbo",
            }

            quotas: list[HostingQuota] = []
            hosted_quota_limit = can20_config.HOSTED_AZURE_OPENAI_QUOTA_LIMIT
    ...
    def init_openai(self) -> HostingProvider:
        ...
        if can20_config.HOSTED_OPENAI_TRIAL_ENABLED:
            hosted_quota_limit = can20_config.HOSTED_OPENAI_QUOTA_LIMIT
        ...
        if can20_config.HOSTED_OPENAI_PAID_ENABLED:
        ...
        if len(quotas) > 0:
            credentials = {
                "openai_api_key": can20_config.HOSTED_OPENAI_API_KEY,
            }

            if can20_config.HOSTED_OPENAI_API_BASE:
                credentials["openai_api_base"] = can20_config.HOSTED_OPENAI_API_BASE

            if can20_config.HOSTED_OPENAI_API_ORGANIZATION:
                credentials["openai_organization"] = can20_config.HOSTED_OPENAI_API_ORGANIZATION
    ...
    def init_anthropic() -> HostingProvider:
        ...
        if can20_config.HOSTED_ANTHROPIC_TRIAL_ENABLED:
            hosted_quota_limit = can20_config.HOSTED_ANTHROPIC_QUOTA_LIMIT
            trial_quota = TrialHostingQuota(quota_limit=hosted_quota_limit)
            quotas.append(trial_quota)

        if can20_config.HOSTED_ANTHROPIC_PAID_ENABLED:
            paid_quota = PaidHostingQuota()
            quotas.append(paid_quota)

        if len(quotas) > 0:
            credentials = {
                "anthropic_api_key": can20_config.HOSTED_ANTHROPIC_API_KEY,
            }

            if can20_config.HOSTED_ANTHROPIC_API_BASE:
                credentials["anthropic_api_url"] = can20_config.HOSTED_ANTHROPIC_API_BASE
    ...
    def init_minimax() -> HostingProvider:
        quota_unit = QuotaUnit.TOKENS
        if can20_config.HOSTED_MINIMAX_ENABLED:
    ...
    def init_spark() -> HostingProvider:
        quota_unit = QuotaUnit.TOKENS
        if can20_config.HOSTED_SPARK_ENABLED:
            quotas: list[HostingQuota] = [FreeHostingQuota()]
    ...
    def init_zhipuai() -> HostingProvider:
        quota_unit = QuotaUnit.TOKENS
        if can20_config.HOSTED_ZHIPUAI_ENABLED:
            quotas: list[HostingQuota] = [FreeHostingQuota()]
    ...
    def init_moderation_config() -> HostedModerationConfig:
        if can20_config.HOSTED_MODERATION_ENABLED and can20_config.HOSTED_MODERATION_PROVIDERS:
            return HostedModerationConfig(enabled=True, providers=can20_config.HOSTED_MODERATION_PROVIDERS.split(","))

        return HostedModerationConfig(enabled=False)

    @staticmethod
    def parse_restrict_models_from_env(env_var: str) -> list[RestrictModel]:
        models_str = can20_config.model_dump().get(env_var)
