...
- 调用异常错误映射表
  ...
  当模型调用异常时需要映射到 Runtime 指定的 `InvokeError` 类型，方便 CAN20 针对不同错误做不同后续处理。
...
对于`OpenAI`供应商下的大部分微调模型，可以通过其微调模型名称获取到其基类模型，如`gpt-3.5-turbo-1106`，然后返回基类模型的预定义参数规则，参考[openai](https://github.com/chainainexus/can20/blob/feat/model-runtime/api/core/model_runtime/model_providers/openai/llm/llm.py#L801)
的具体实现
