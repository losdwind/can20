...
When a model call fails, map it to a specific `InvokeError` type as required by Runtime, allowing CAN20 to handle different errors accordingly.
...
For interface method explanations, see: [Interfaces](./interfaces.md). For detailed implementation, refer to: [llm.py](https://github.com/chainainexus/can20-runtime/blob/main/lib/model_providers/anthropic/llm/llm.py).